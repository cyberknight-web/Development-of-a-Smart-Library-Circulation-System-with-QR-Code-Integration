...
$stmt_items = $pdo->prepare(
    "SELECT bri.*, b.title FROM borrow_request_items bri JOIN books b ON b.id = bri.book_id WHERE bri.borrow_request_id = :id"
);
$stmt_items->execute([':id' => $record['id']]);
        ...
        <div class="card sl-card">
            <div class="card-body">
                ...
                <dl class="row small mb-0">
                    ...
                    <dd class="col-sm-8"><?php echo htmlspecialchars($record['name'], ENT_QUOTES, 'UTF-8'); ?></dd>
                    <dt class="col-sm-4">Student ID</dt>
                    <dd class="col-sm-8"><?php echo htmlspecialchars($record['student_code'], ENT_QUOTES, 'UTF-8'); ?></dd>
                    <dt class="col-sm-4">Course / Section</dt>
                    <dd class="col-sm-8"><?php echo htmlspecialchars($record['course'], ENT_QUOTES, 'UTF-8'); ?> / <?php echo htmlspecialchars($record['section'], ENT_QUOTES, 'UTF-8'); ?></dd>
                    <dt class="col-sm-4">Email</dt>
                    <dd class="col-sm-8"><?php echo htmlspecialchars($record['email'], ENT_QUOTES, 'UTF-8'); ?></dd>
                    <dt class="col-sm-4">Status</dt>
                    <dd class="col-sm-8"><span class="badge bg-warning"><?php echo htmlspecialchars($record['status'], ENT_QUOTES, 'UTF-8'); ?></span></dd>
                    <?php if ($record['notes']): ?>
                    <dt class="col-sm-4">Notes</dt>
                    <dd class="col-sm-8"><span class="text-break"><?php echo htmlspecialchars($record['notes'], ENT_QUOTES, 'UTF-8'); ?></span></dd>
                    <?php endif; ?>
                </dl>
                <h6 class="mt-3">Books</h6>
                <ul class="list-group list-group-flush">
                    <?php foreach ($items as $item): ?>
                        <li class="list-group-item px-0"><?php echo htmlspecialchars($item['title'], ENT_QUOTES, 'UTF-8'); ?></li>
                    <?php endforeach; ?>
                </ul>
                <p class="text-muted small mt-3 mb-0">Return within 3 days. Present this QR at the library for the admin to scan when returning.</p>
            </div>
        </div>
    </div>
</div>
